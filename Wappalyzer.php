...
class Wappalyzer
{
	...
	public function __construct($url)
	{
		...
		$json = json_decode(file_get_contents('Wappalyzer/src/apps.json'));
	...
	{
		try {
			$this->load(array('Wappalyzer/src/wappalyzer.js', 'js/driver.js'));
	...
	protected function load($files)
	{
		...
		foreach ( $files as $file ) {
			$this->v8->executeString(file_get_contents($file), $file);
		}
	}
